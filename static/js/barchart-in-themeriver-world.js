...
const personSort = function(b,a){
    return (a['confirmed cases']) - (b['confirmed cases']);
}
...
const barValue = d => { return(d['confirmed cases']); }; 
let barScale;
...
const setting_up_barchart = function(data){
    ...
    let intervalId = setInterval(function(){
        if(c >= alldates.length){
            ...
            date = alldates[c];
            let data_today = data.filter(datum => {return datum['date'] === date});
            data_today = data_today.filter(datum => {return datum['confirmed cases'] !== 0});
...
const barchart_color = {
    'existing diagnosis': '#6cd7e6',
    'confirmed cases': '#6cd7e6',
    'deaths': '#6be86b',
    'death toll': '#df5353'
}
...
const renderbarchart = function(data){
    ...
    .attr('y', function (d,i) {
        return y - barScale(d['confirmed cases']);
    })
    .attr('width', 15)
    .attr('height', d => barScale(d['confirmed cases']))
    ...
    .attr('y', function (d,i) {
        return y - barScale(d['confirmed cases']) - 5;
    })
    .style('font-size', '.7em')
    .attr('text-anchor', 'middle')
    .text(d => d['confirmed cases'])
    .attr('opacity', 0.)
    .transition().ease(d3.easeCubic)
    .attr('opacity', 1.);
